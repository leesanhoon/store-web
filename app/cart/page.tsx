...
export default function CartPage() {
  ...
  const [form, setForm] = useState<QuoteForm>(() => getInitialForm(getCartItems()));
  ...
  const reloadCart = () => {
    ...
    setForm((current) => ({
      ...current,
      productInterest: current.productInterest || nextItems[0]?.name || "",
      quantity: current.quantity || (nextItems[0]?.quantity ? String(nextItems[0].quantity) : ""),
    }));
  };
    ...
    <MobileAppShell>
      <div className="quote-screen">
            ...
            <div className="quote-items">
              {items.map((item) => (
                <article key={getCartItemKey(item)}>
                  {item.imageSrc ? (
                    <Image src={item.imageSrc} alt={item.name} width={52} height={52} />
                  ) : null}
                  <div>
                    <strong>{item.name}</strong>
                    <span>
                      {item.quantity.toLocaleString("vi-VN")} {formatUnit(item.unit)} · {item.configuration.printMethod}
                      {item.configuration.lidOption ? ` · ${item.configuration.lidOption}` : ""}
                    </span>
                    {item.configuration.note ? <span>{item.configuration.note}</span> : null}
                  </div>
        ...
        <section className="why-card">
          ...
          <Image src="/images/mockups/logo-cup-500-urban.png" alt="Ly giấy in logo" width={320} height={360} />
        </section>
