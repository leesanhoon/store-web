        ...
        <div className="space-y-3 text-[#101a36]">
                    ...
                    <div className="mt-2.5 overflow-hidden rounded-[14px] border border-[#f1e7d8] bg-white">
                        {products.slice(0, 5).map((product) => {
                            ...
                            return (
                                <article
                                    key={product.id}
                                    className="grid grid-cols-[1fr_auto] gap-x-2 gap-y-1 border-b border-[#f1e7d8] px-3 py-2.5 last:border-b-0"
                                >
                                    ...
                                    <p className="text-[12px] font-semibold text-[#3d4860]">
            ...
            productsWithoutImage.length > 0 ? (
                <AdminCard className="border-amber-200 bg-amber-50 p-3.5">
                    <h2 className="text-[14px] font-extrabold text-[#101a36]">
                        Cảnh báo vận hành
                    </h2>
                    <ul className="mt-2 space-y-1.5 text-[12px] font-semibold text-[#3d4860]">
                        {productsWithoutPrice.length > 0 ? (
                            <li>
                                {productsWithoutPrice.length} sản phẩm chưa có
                                bảng giá (biến thể).
                            </li>
                        ) : null}
                        {productsWithoutImage.length > 0 ? (
                            <li>
                                {productsWithoutImage.length} sản phẩm thiếu ảnh
                                đại diện.
                            </li>
                        ) : null}
                    </ul>
