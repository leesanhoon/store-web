    ...
    useEffect(() => {
        void (async () => {
            ...
            try {
                ...
                setError(err instanceof Error ? err.message : "Kh�ng th? t?i chi ti?t don.");
                setDetail(null);
            } finally {
                setLoading(false);
            }
    ...
    return (
        <div className="space-y-6">
            <section className="panel-strong bg-[#111] p-7 text-white">
                ...
                <h1 className="mt-2 text-3xl font-semibold">Qu?n l� don h�ng</h1>
                <p className="mt-2 text-sm text-white/70">D? li?u l?y tr?c ti?p t? API Orders.</p>
            </section>

            <section className="grid gap-4 xl:grid-cols-[360px_1fr]">
                <aside className="rounded-3xl border border-[#e6e0d8] bg-white p-5">
                    <h2 className="text-lg font-semibold text-header">Danh s�ch don</h2>
                    <div className="mt-4 space-y-3">
                        {orders.length === 0 ? (
                            <p className="text-sm font-medium text-slate-600">Chua c� don n�o.</p>
                        ) : (
                            orders.map((order) => (
                                <button
                                    key={order.id}
                    ...
                    <div className="mt-4">
                        <Link href="/track-order" className="button-secondary w-full">Tra c?u don</Link>
                    </div>
                </aside>

                <div className="space-y-6">
                    {loading ? <div className="panel p-6 text-sm text-slate-500">�ang t?i chi ti?t...</div> : null}
                    ...
                    {detail ? (
                        <article className="panel p-6">
                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-slate-600">{detail.customerPhone || "Chua c� s?"}</p>
                        ...
                        <div className="panel p-6 text-sm font-medium text-slate-600">{selectedSummary ? "Kh�ng t?i du?c chi ti?t." : "Chua c� don du?c ch?n."}</div>
                    )}
                </div>
            </section>
        </div>
