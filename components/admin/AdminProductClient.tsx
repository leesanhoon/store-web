...
const statusOptions: ProductStatus[] = ["Đang hiển thị", "Ẩn", "Hết hàng", "Ngừng bán"];
...
function EditIcon() {
  return <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" aria-hidden="true"><path d="m4 20 4.5-1 10-10a2.1 2.1 0 0 0 0-3l-.5-.5a2.1 2.1 0 0 0-3 0l-10 10L4 20Z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" /></svg>;
}

function DeleteIcon() {
  return <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" aria-hidden="true"><path d="M6 7h12M10 11v6M14 11v6M8 7l1 13h6l1-13M10 7V5h4v2" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}

function CopyIcon() {
  return <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" aria-hidden="true"><path d="M8 8h10v12H8V8Z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" /><path d="M6 16H4V4h10v2" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}

function PlusIcon() {
  return <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" aria-hidden="true"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" /></svg>;
}
...
function FieldLabel({ children, required }: { children: ReactNode; required?: boolean }) {
  return <span className="mb-1.5 block text-[13px] font-extrabold text-[#0b1b3b]">{children} {required ? <span className="text-red-500">*</span> : null}</span>;
}
...
function AdminGalleryPicker({
  ...
  return (
    <div className="grid grid-cols-3 gap-2 rounded-[14px] border border-[#eadfce] bg-white p-2">
      ...
      <button type="button" onClick={onOpenPicker} className="grid h-[68px] place-items-center rounded-[10px] border border-dashed border-[#d6c9b8] bg-white text-center text-[12px] font-extrabold text-[#0b1b3b] transition active:scale-[0.98]">
...
export default function AdminProductClient({ initialProducts, initialCategories }: Props) {
  ...
  const categorySelectOptions = categoryOptions.map((category) => ({ value: String(category.id), label: category.name }));
  ...
  const visibleProducts = listProducts.filter((product) => {
    ...
    const text = normalizeSearch(`${product.name} ${product.description ?? ""} ${product.categoryName} ${status}`);
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (selectedId) {
        ...
        setProductStatuses((current) => ({ ...current, [selectedId]: form.status }));
      ...
      <div className="admin-product-form text-[#0b1b3b]">
        <div className="flex items-center gap-3">
          <button type="button" onClick={closeForm} className="grid h-10 w-10 place-items-center rounded-full text-[#0b1b3b]" aria-label="Quay lại">
            <svg viewBox="0 0 24 24" fill="none" className="h-6 w-6" aria-hidden="true"><path d="M15 5 8 12l7 7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
          </button>
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-4">
          <label className="block"><FieldLabel required>Tên sản phẩm</FieldLabel><AdminField value={form.name} onChange={(event) => setForm((prev) => ({ ...prev, name: event.target.value }))} placeholder="Nhập tên sản phẩm" /></label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block"><FieldLabel required>Giá từ (đ)</FieldLabel><AdminField type="number" value={form.price} onChange={(event) => setForm((prev) => ({ ...prev, price: event.target.value }))} placeholder="Ví dụ: 850" /></label>
            <label className="block"><FieldLabel required>MOQ</FieldLabel><AdminField type="number" value={form.stockQuantity} onChange={(event) => setForm((prev) => ({ ...prev, stockQuantity: event.target.value }))} placeholder="Ví dụ: 1000" /></label>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <label className="block"><FieldLabel>Dung tích</FieldLabel><AdminSelect value={form.volume} onValueChange={(value) => setForm((prev) => ({ ...prev, volume: value }))} placeholder="Chọn dung tích" options={volumeOptions.map((item) => ({ value: item, label: item }))} /></label>
            <label className="block"><FieldLabel>Trạng thái</FieldLabel><AdminSelect value={form.status} onValueChange={(value) => setForm((prev) => ({ ...prev, status: value as ProductStatus }))} options={statusOptions.map((item) => ({ value: item, label: item }))} /></label>
          </div>
          <label className="block"><FieldLabel>Mô tả sản phẩm</FieldLabel><AdminTextArea rows={2} value={form.description} onChange={(event) => setForm((prev) => ({ ...prev, description: event.target.value }))} placeholder="Nhập mô tả sản phẩm..." /></label>

          <AdminCard className="space-y-3 p-3.5">
            <h2 className="text-[14px] font-extrabold">Cấu hình bán hàng</h2>
            <label className="block"><FieldLabel>Nắp tương thích</FieldLabel><AdminSelect value={form.lidOptions} onValueChange={(value) => setForm((prev) => ({ ...prev, lidOptions: value }))} placeholder="Chọn nắp" options={lidOptionList.map((item) => ({ value: item, label: item }))} /></label>
            <label className="block"><FieldLabel>Bảng giá theo số lượng</FieldLabel><AdminTextArea rows={2} value={form.priceTiers} onChange={(event) => setForm((prev) => ({ ...prev, priceTiers: event.target.value }))} placeholder="Ví dụ: 1.000 sp: 850đ, 5.000 sp: 780đ" /></label>
            <p className="text-[11px] font-semibold text-slate-500">Các trường cấu hình đang lưu ở UI để chờ backend hỗ trợ, không gửi vào payload sản phẩm hiện tại.</p>
          </AdminCard>
  ...
  return (
    <div className="space-y-3 text-[#0b1b3b]">
      <AdminSectionHeader title="Quản lý sản phẩm" action={<AdminPrimaryButton type="button" onClick={startCreate} className="h-10 min-h-10 rounded-[13px] px-3 text-[14px]"><PlusIcon />Thêm</AdminPrimaryButton>} />
      <label className="flex h-12 items-center gap-3 rounded-[14px] border border-[#eadfce] bg-white px-3.5 text-slate-400 shadow-sm">
        ...
        <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Tìm sản phẩm, trạng thái..." className="min-w-0 flex-1 bg-transparent text-[14px] font-semibold text-[#0b1b3b] outline-none placeholder:text-slate-400" />
      </label>
              ...
              <div className="grid grid-cols-[92px_1fr_auto] items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <span className="rounded-lg bg-[#f8f0e6] px-2 py-1 text-[10px] font-extrabold text-[#3d4860]">Logo theo yêu cầu</span>
                  </div>
                </div>
                <div className="flex flex-col gap-2">
                  <IconButton label="Sửa sản phẩm" onClick={() => editProduct(product)}><EditIcon /></IconButton>
                  <IconButton label="Nhân bản sản phẩm" onClick={() => cloneProduct(product)}><CopyIcon /></IconButton>
