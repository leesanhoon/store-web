...
const emptyVariant: VariantRow = { capacityMl: "", diameterMm: "", priceTiers: [{ ...emptyPriceTier }] };
...
function AdminImageUploadBox({ inputRef, previewUrl, onFileChange, onOpenPicker }: {
    ...
    return (
        <div>
            <input ref={inputRef} type="file" accept=".jpg,.jpeg,.png,.webp,.gif,image/*" onChange={onFileChange} className="hidden" />
            <button type="button" onClick={onOpenPicker} className="grid min-h-[136px] w-full place-items-center rounded-[14px] border border-dashed border-[#8a99ad] bg-white/60 p-3 text-center text-[#3d4860] transition active:scale-[0.99] min-[431px]:min-h-[154px]">
                {previewUrl ? (
                    <Image src={previewUrl} alt="Ảnh đại diện xem trước" width={220} height={180} unoptimized className="h-[118px] w-full rounded-xl object-cover min-[431px]:h-[136px]" />
                ) : (
...
function AdminGalleryPicker({ inputRef, imageSources, onFileChange, onOpenPicker }: {
    inputRef: RefObject<HTMLInputElement | null>;
    imageSources: string[];
    onFileChange: (event: React.ChangeEvent<HTMLInputElement>) => void;
    onOpenPicker: () => void;
    ...
    return (
        <div className="grid grid-cols-3 gap-2 rounded-[14px] border border-[#eadfce] bg-white p-2">
            <input ref={inputRef} type="file" accept=".jpg,.jpeg,.png,.webp,.gif,image/*" multiple onChange={onFileChange} className="hidden" />
            {imageSources.slice(0, 5).map((src, index) => (
                <Image key={`${src}-${index}`} src={src} alt="Ảnh sản phẩm" width={96} height={86} unoptimized={src.startsWith("blob:")} className="h-[68px] w-full rounded-[10px] border border-[#f1e7d8] object-cover" />
            ))}
            <button type="button" onClick={onOpenPicker} className="grid h-[68px] place-items-center rounded-[10px] border border-dashed border-[#d6c9b8] bg-white text-center text-[12px] font-extrabold text-[#101a36] transition active:scale-[0.98]">
                <span><span className="block text-2xl leading-none">+</span>Ảnh khác</span>
            </button>
        </div>
    );
}
...
function VariantEditor({ variants, onChange }: {
    ...
    const addVariant = () => {
        onChange([...variants, { capacityMl: "", diameterMm: "", priceTiers: [{ ...emptyPriceTier }] }]);
    };
    ...
    const updateVariant = (index: number, field: keyof Omit<VariantRow, "priceTiers">, value: string) => {
        onChange(variants.map((v, i) => (i === index ? { ...v, [field]: value } : v)));
    };

    const updatePriceTiers = (vIndex: number, tiers: PriceTierRow[]) => {
        onChange(variants.map((v, i) => (i === vIndex ? { ...v, priceTiers: tiers } : v)));
    };
    ...
    const removePriceTier = (vIndex: number, tIndex: number) => {
        ...
        updatePriceTiers(vIndex, v.priceTiers.filter((_, i) => i !== tIndex));
    };

    const updateTier = (vIndex: number, tIndex: number, field: keyof PriceTierRow, value: string) => {
        const v = variants[vIndex];
        updatePriceTiers(vIndex, v.priceTiers.map((t, i) => (i === tIndex ? { ...t, [field]: value } : t)));
    };

    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between">
                <h2 className="text-[14px] font-extrabold">Biến thể sản phẩm</h2>
                <button type="button" onClick={addVariant} className="inline-flex items-center gap-1 rounded-lg bg-[#101a36] px-2.5 py-1.5 text-[11px] font-extrabold text-white">
                    + Thêm biến thể
                </button>
            </div>
            {variants.map((variant, vIndex) => (
                <AdminCard key={vIndex} className="space-y-3 p-3">
                    <div className="flex items-center justify-between">
                        <span className="text-[13px] font-extrabold text-[#101a36]">Biến thể {vIndex + 1}</span>
                        {variants.length > 1 ? (
                            <button type="button" onClick={() => removeVariant(vIndex)} className="text-[11px] font-bold text-rose-600">Xóa</button>
                        ) : null}
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                        <label className="block">
                            <span className="mb-1 block text-[11px] font-bold text-slate-500">Dung tích (ml)</span>
                            <AdminField type="number" value={variant.capacityMl} onChange={(e) => updateVariant(vIndex, "capacityMl", e.target.value)} placeholder="250" />
                        </label>
                        <label className="block">
                            <span className="mb-1 block text-[11px] font-bold text-slate-500">⌀ miệng (mm)</span>
                            <AdminField type="number" value={variant.diameterMm} onChange={(e) => updateVariant(vIndex, "diameterMm", e.target.value)} placeholder="90" />
                        </label>
                    </div>
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            <span className="text-[12px] font-bold text-slate-600">Bảng giá theo số lượng</span>
                            <button type="button" onClick={() => addPriceTier(vIndex)} className="text-[11px] font-bold text-emerald-700">+ Thêm mức giá</button>
                        </div>
                        {variant.priceTiers.map((tier, tIndex) => (
                            <div key={tIndex} className="grid grid-cols-[1fr_1fr_32px] gap-2 items-end">
                                <label className="block">
                                    {tIndex === 0 ? <span className="mb-1 block text-[10px] font-bold text-slate-500">Từ SL</span> : null}
                                    <AdminField type="number" value={tier.minQuantity} onChange={(e) => updateTier(vIndex, tIndex, "minQuantity", e.target.value)} placeholder="1000" />
                                </label>
                                <label className="block">
                                    {tIndex === 0 ? <span className="mb-1 block text-[10px] font-bold text-slate-500">Đơn giá (đ)</span> : null}
                                    <AdminField type="number" value={tier.unitPrice} onChange={(e) => updateTier(vIndex, tIndex, "unitPrice", e.target.value)} placeholder="850" />
                                </label>
                                <button type="button" onClick={() => removePriceTier(vIndex, tIndex)} className="grid h-[44px] w-8 place-items-center rounded-xl text-rose-500 hover:bg-rose-50" aria-label="Xóa mức giá">
                                    ×
                                </button>
                            </div>
                        ))}
...
function LidSelector({ selectedIds, allLids, onChange }: {
    selectedIds: number[];
    allLids: LidDto[];
    ...
    if (allLids.length === 0) {
        return (
            <p className="text-[12px] font-semibold text-slate-500">Chưa có nắp nào trong hệ thống. Hãy tạo nắp trước.</p>
        );
    }
    ...
    return (
        <div className="space-y-1.5">
            {allLids.map((lid) => {
                ...
                return (
                    <button key={lid.id} type="button" onClick={() => toggle(lid.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition ${checked ? "bg-emerald-50 ring-1 ring-emerald-300" : "bg-[#fffaf2]"}`}>
                        <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-md text-[11px] ${checked ? "bg-emerald-600 text-white" : "bg-white ring-1 ring-slate-300"}`}>
                            {checked ? "✓" : ""}
                        </span>
                        <span className="min-w-0">
                            <span className="block truncate text-[13px] font-bold text-[#101a36]">{lid.name}</span>
                            <span className="block text-[10px] font-semibold text-slate-500">
                                {lid.prices.map((p) => `⌀${p.diameterMm}mm`).join(", ")}
                            </span>
                        </span>
                    </button>
                );
...
export default function AdminProductClient({ initialProducts, initialCategories }: Props) {
    ...
    const { data: allLids = [] } = useSWR<LidDto[]>("/api/v1/Lids", getLids, { fallbackData: [] });
    ...
    const openAvatarPicker = () => { preserveAdminScroll(); avatarInputRef.current?.click(); };
    const openGalleryPicker = () => { preserveAdminScroll(); galleryInputRef.current?.click(); };
    const handleAvatarFileChange = (event: React.ChangeEvent<HTMLInputElement>) => { setAvatarImage(event.currentTarget.files?.[0] ?? null); event.currentTarget.value = ""; preserveAdminScroll(); };
    const handleGalleryFileChange = (event: React.ChangeEvent<HTMLInputElement>) => { setGalleryImages(Array.from(event.currentTarget.files ?? [])); event.currentTarget.value = ""; preserveAdminScroll(); };
        ...
        setForm({
            ...
            variants: product.variants.length > 0
                ? product.variants.map((v) => ({
                    ...
                    priceTiers: v.priceTiers.length > 0
                        ? v.priceTiers.map((t) => ({ minQuantity: String(t.minQuantity), unitPrice: String(t.unitPrice) }))
                        : [{ ...emptyPriceTier }],
                }))
                : [{ ...emptyVariant, priceTiers: [{ ...emptyPriceTier }] }],
            ...
            <div className="admin-product-form text-[#101a36]">
                ...
                <form onSubmit={onSubmit} className="mt-4 space-y-4">
                    <label className="block">
                        ...
                        <AdminField value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} placeholder="Nhập tên sản phẩm" />
                    </label>
                    <label className="block">
                    ...
                    <label className="block">
                        ...
                        <AdminTextArea rows={2} value={form.description} onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))} placeholder="Nhập mô tả sản phẩm..." />
                    </label>
                    ...
                    <AdminCard className="space-y-3 p-3.5">
                        <h2 className="text-[14px] font-extrabold">Nắp tương thích</h2>
                        <LidSelector selectedIds={form.lidIds} allLids={allLids} onChange={(lidIds) => setForm((prev) => ({ ...prev, lidIds }))} />
                    </AdminCard>
                    ...
                    {!selectedId ? (
                        <div className="grid gap-4 min-[431px]:grid-cols-[0.95fr_1.05fr]">
                            <div>
                                ...
                                <AdminImageUploadBox inputRef={avatarInputRef} previewUrl={avatarPreviewUrl} onFileChange={handleAvatarFileChange} onOpenPicker={openAvatarPicker} />
                            </div>
                            <div>
                                <FieldLabel>Thư viện ảnh</FieldLabel>
                                <AdminGalleryPicker inputRef={galleryInputRef} imageSources={galleryImageSources} onFileChange={handleGalleryFileChange} onOpenPicker={openGalleryPicker} />
                            </div>
                        </div>
                    ) : null}
    ...
    return (
        <div className="space-y-3 text-[#101a36]">
            ...
            <label className="flex h-12 items-center gap-3 rounded-[14px] border border-[#eadfce] bg-white px-3.5 text-slate-400 shadow-sm">
                ...
                <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Tìm sản phẩm..." className="min-w-0 flex-1 bg-transparent text-[14px] font-semibold text-[#101a36] outline-none placeholder:text-slate-400" />
            </label>
            <div className="flex gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {tabs.map((tab) => (
                    <AdminChip key={tab} active={activeTab === tab} onClick={() => setActiveTab(tab)}>{tab}</AdminChip>
                ))}
            </div>
            {productsError ? <AdminCard className="p-3 text-[12px] font-semibold text-rose-700">Không tải được sản phẩm.</AdminCard> : null}
                        ...
                        <AdminCard key={product.id} className="p-3">
                            <div className="grid grid-cols-[92px_1fr_auto] items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <h2 className="truncate text-[17px] font-extrabold leading-tight text-[#101a36]">{product.name}</h2>
                                    ...
                                    <div className="mt-2 grid grid-cols-2 gap-3 text-[12px] font-semibold text-[#3d4860]">
                                        <div>
                                            <span className="block">Giá từ</span>
                                            <b className="text-[14px] text-[#101a36]">{minPrice !== null ? adminFormatMoney(minPrice) : "Liên hệ"}</b>
                                        </div>
                                        <div>
                                            <span className="block">Biến thể</span>
                                            <b className="text-[14px] text-[#101a36]">{variantCount} loại</b>
                                        </div>
                                    </div>
                                    <div className="mt-2 flex flex-wrap items-center gap-2">
                                        ...
                                        {variantCount > 0 ? (
                                            <span className="rounded-lg bg-[#f8f0e6] px-2 py-1 text-[10px] font-extrabold text-[#3d4860]">
                                                {product.variants.map((v) => `${v.capacityMl}ml`).join(", ")}
                                            </span>
                                        ) : null}
                                    </div>
                                </div>
                                <div className="flex flex-col gap-2">
                                    <IconButton label="Sửa sản phẩm" onClick={() => editProduct(product)}>
                                        <EditIcon />
                                    </IconButton>
                                    <IconButton label="Xóa sản phẩm" onClick={() => void handleDelete(product.id)}>
                                        <DeleteIcon />
                                    </IconButton>
                                </div>
                            </div>
